...
function draw() {
  background(180);
  text("Puntuación: "+ score, width/2+150,height/8);
  ...
  if(gameState === PLAY){
    ...
    if((c || keyDown("space"))  && trex.y >= height-110 ) {
      //trex.velocityY = -13;
      trex.velocityY = -13;
...
function spawnClouds() {
  ...
  if (frameCount % 60 === 0) {
   ...
   //neblina 
    ...
    if (frameCount % 60  === 0){
      cloud2 = createSprite(width+20,height-300,width,10);
    cloud2.addImage(cloudImage2);
    cloud2.y = Math.round(random(270,580));
    cloud2.scale = 1.55;
    cloud2.velocityX = -3;
...
function spawnObstacles(){
 if (frameCount % 60 === 0){
   var obstacle = createSprite(width+20,height-20,width,10);
...
function SpawnObsMe(){
  if (frameCount % 90 === 0){
    var obsMe = createSprite(width/2,height-400,width,15);
